...
import ScrollGallery from "@/components/works/ScrollGallery";
import { gridCols } from "@/components/works/galleryStyles";
    ...
    <PageShell>
      {scrollGroup ? (
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_2fr] lg:items-start">
          <WorkInfo work={work} />
          <ScrollGallery group={scrollGroup} />
        </div>
      ) : (
        <>
          <WorkInfo work={work} />
          ...
          {work.gallery.some(
            ...
            <section className="flex flex-col gap-4 rounded-2xl border-[0.5px] border-main bg-white p-5 md:p-8">
              {work.gallery.map((group, gi) => {
                ...
                const isLPGroup =
                  flat.length > 0 &&
                  flat.every((img) => img.item === "WEBサイト");
                if (isLPGroup) {
                  return (
